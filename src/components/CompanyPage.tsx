...
import { CheckCircle, FileDown, Mail, ArrowRight, Sparkles, Home, Linkedin } from 'lucide-react';
import { useSearchParams } from 'next/navigation';
import Link from 'next/link';
import { useEffect, useState } from 'react';
import { COMPANY_PAGE_ENDPOINT } from '@/config/companyPage';

type Lane = 'gtm' | 'cs' | 'ai' | 'healthcare';
...
const RESUME_BY_LANE: Record<Lane, string> = {
  gtm: '/Marcus_Hansen_CS_Resume.pdf',
  cs: '/Marcus_Hansen_CS_Resume.pdf',
  ai: '/Marcus_Hansen_Resume.pdf',
  healthcare: '/Marcus_Hansen_Healthcare_Resume.pdf',
};

const PAGE_BY_LANE: Record<Lane, string> = {
  gtm: '/gtm',
  cs: '/customer-success',
  ai: '/ai',
...
const LINKEDIN = 'https://www.linkedin.com/in/marcus-hansen-39756326b/';
...
export default function CompanyPage() {
  ...
  const mode = (params.get('mode') || '').toLowerCase() === 'peer' ? 'peer' : 'pitch';
  ...
  const steps =
    mode === 'peer'
      ? [`Reading about ${company || 'them'}...`, 'Putting a note together...', 'Almost there...']
      : [`Reading about ${company || 'your company'}...`, "Mapping Marcus's experience to your world...", 'Assembling your page...'];
  ...
  useEffect(() => {
    ...
    const url =
      `${COMPANY_PAGE_ENDPOINT}?c=${encodeURIComponent(company)}` +
      (role ? `&r=${encodeURIComponent(role)}` : '') +
      (mode === 'peer' ? '&mode=peer' : '');
    fetch(url)
      .then(async (res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json();
        if (!json.headline) throw new Error('Bad payload');
    ...
    return (
      <Shell>
        ...
        <p className="text-lg text-ink-soft max-w-2xl mx-auto mb-10">
          When Marcus reaches out to a company, this page builds itself for their team.
  ...
  if (failed || !COMPANY_PAGE_ENDPOINT) {
    return (
      <Shell>
        ...
        <p className="text-lg text-ink-soft max-w-2xl mx-auto mb-10">
          This page is having a moment, but Marcus is very real. You can see his full site,
          or reach him at{' '}
  ...
  const lanePage = PAGE_BY_LANE[data.lane] || '/';

  // ---- PEER page: curiosity-first, no pitch, no resume ----
  if (data.mode === 'peer') {
    return (
      <main className="min-h-screen w-full bg-cream">
        <div className="max-w-2xl mx-auto px-6 py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="font-serif text-4xl md:text-5xl font-semibold mb-8 text-ink">
              {data.headline}
            </h1>
            <p className="text-lg md:text-xl text-ink-soft leading-relaxed mb-8">{data.intro}</p>

            <div className="bg-white rounded-2xl p-8 border border-line shadow-[0_2px_16px_rgba(45,42,38,0.05)] mb-8">
              <p className="text-ink-soft leading-relaxed mb-4">{data.aboutMe}</p>
              <p className="text-ink-soft leading-relaxed">{data.whyCurious}</p>
            </div>

            <p className="text-lg text-ink-soft leading-relaxed mb-10">{data.closing}</p>
  ...
  // ---- PITCH page: fit points + resume (applications + hiring managers) ----
  return (
    <main className="min-h-screen w-full bg-cream">
      <div className="max-w-5xl mx-auto px-6 py-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-center mb-16">
          <img
            src="/marcus-avatar.jpg"
            alt="Marcus Hansen"
            className="w-20 h-20 rounded-full object-cover mx-auto mb-6 border-4 border-white shadow-[0_2px_16px_rgba(45,42,38,0.12)]"
          />
          <p className="text-clay uppercase tracking-widest text-sm mb-4 font-medium">
